...
for key, value in comNums.items():
    print(f'Found {value} records for {key}.')
...
sorted_commodities = sorted(comNums.items(), key=lambda a: a[1])
# (7) print the commodity with the fewest number of records
# (one source code statement)
print(f'Least available commodity = {sorted_commodities[0][0]}.')
...
print(f'{sorted_commodities[0][0]} were available for {int(sorted_commodities[0][1]/5)} weeks.')
...
# (one source code statement)
min1 = min([i[-1] for i in data if (i[0].lower() == 'nectarines' and i[-2].lower() == 'chicago')])
...
mind1 = [i[1] for i in data if i[-1] == min1 and (i[0].lower() == 'nectarines' and i[-2].lower() == 'chicago')]
# (11) print the lowest price for nectarines in Chicago and the date
# (one source code statement)
[print(f'The lowest price for Nectarines in Chicago was ${min1} on {j}.') for j in mind1]
# (12) use a list comprehension to find the farm price for nectarines on the same
# date and assign it to a variable named fp1
# (one source code statement)
fp1 = [i[-1] for i in data if (i[0].lower() == 'nectarines' and i[-2].lower() == 'farm' and i[1] == min(mind1))]
# (13) print the farm price for nectarines on the date
# of lowest price in Chicago and print the computed price difference
# (one source code statement)
print(f'Nectarine farm price on {mind1[0]} was ${min(fp1):.2f}; ${min1- min(fp1):.2f} less than in Chicago')
...
comNumsDict = dict([(i, int(comList.count(i) / 5)) for i in uComList])
...
pAvg = sum([i[-1] for i in data if i[0] == 'Peaches' and i[-2] == 'Atlanta']) / comNumsDict['Peaches']
